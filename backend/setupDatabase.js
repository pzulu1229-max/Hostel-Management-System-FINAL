const pool = require('./db');

async function setupDatabase() {
  try {
    console.log('🔧 Setting up database...');
    
    // Create allocations table
    await pool.query(`
      CREATE TABLE IF NOT EXISTS allocations (
        id SERIAL PRIMARY KEY,
        user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
        room_id INTEGER REFERENCES rooms(id) ON DELETE CASCADE,
        booking_date DATE DEFAULT CURRENT_DATE,
        check_in DATE,
        check_out DATE,
        status VARCHAR(20) DEFAULT 'active'
      )
    `);
    console.log('✅ Allocations table created');
    
    // Clear existing rooms
    await pool.query('DELETE FROM allocations');
    await pool.query('DELETE FROM rooms');
    console.log('✅ Cleared existing data');
    
    // Add columns if they don't exist
    await pool.query('ALTER TABLE rooms ADD COLUMN IF NOT EXISTS is_available BOOLEAN DEFAULT true');
    await pool.query('ALTER TABLE rooms ADD COLUMN IF NOT EXISTS description TEXT');
    
    // Insert 20 rooms with 4-person capacity at K500
    const roomNumbers = [
      'A101', 'A102', 'A103', 'A104', 'A105',
      'B201', 'B202', 'B203', 'B204', 'B205',
      'C301', 'C302', 'C303', 'C304', 'C305',
      'D401', 'D402', 'D403', 'D404', 'D405'
    ];
    
    const descriptions = [
      'Comfortable 4-bed room with study area and wardrobe',
      'Spacious 4-bed room with attached bathroom and balcony',
      'Modern 4-bed room with AC, TV, and study table',
      'Standard 4-bed shared room with good ventilation',
      'Premium 4-bed room with extra space and modern furniture'
    ];
    
    console.log('📝 Adding 20 rooms...');
    
    for (let i = 0; i < roomNumbers.length; i++) {
      const descIndex = i % descriptions.length;
      await pool.query(`
        INSERT INTO rooms (room_number, capacity, price, status, is_available, description) 
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [roomNumbers[i], 4, 500.00, 'available', true, descriptions[descIndex]]);
      console.log(`  ✅ ${roomNumbers[i]} - 4 persons - K500/month`);
    }
    
    // Verify
    const count = await pool.query('SELECT COUNT(*) FROM rooms');
    console.log(`\n📊 Total rooms: ${count.rows[0].count}`);
    
    const sample = await pool.query('SELECT room_number, capacity, price FROM rooms LIMIT 5');
    console.log('\n📋 Sample rooms:');
    sample.rows.forEach(row => {
      console.log(`   ${row.room_number} - ${row.capacity} persons - K${row.price}`);
    });
    
    console.log('\n✅ Database setup complete!');
    process.exit(0);
  } catch (err) {
    console.error('❌ Error:', err.message);
    process.exit(1);
  }
}

setupDatabase();
